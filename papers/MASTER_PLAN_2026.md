...
## 10. P06 (ods-unified-v2) - continuum closure + context streaming

Working note: `P06_NS_CLOSURE_AND_CONTEXT_STREAMING.md`

### Core claim

P06 should be framed as an effective multiscale closure, not as a proof that NS3D
is solved. The serious claim is:

- ODS can provide a structured closure when continuum validity degrades.
- The trigger should be expressed in terms of `eta`, `Kn_loc`, enstrophy,
  vorticity concentration, texture, and focus.
- The same machinery can be reused for sparse context capture:
  ROI, delta packets, event-style streaming, and slice-aware updates.

### Why this matters

This is the first paper where the fluid / closure story and the context-catching
story become the same systems story:

- detect where the dense continuum/frame representation is failing or wasting budget
- move only the relevant local changes
- update only the affected slices
- reuse texture/focus as the routing policy

### Immediate scope

- no exact NS3D singularity claims
- no Planck rhetoric for ordinary fluids
- yes to operational breakdown criteria and closure tests
- yes to ROI / delta / event-style context capture
...
## 11. Serie CT Dimensional — Slices Cl(p,q) sobre Gamma (P04/P06 futuro)

**Fuente**: CT_STRUCTURAL_GENOME_LANGUAGE_V0.md + kernel_field/backends.py

### La serie definida

| Slice | Algebra    | Rol operativo                          | Componentes |
|-------|------------|----------------------------------------|-------------|
| 0     | Cl(1,0)    | Gate minimo, activacion binaria        | 2           |
| 1     | Cl(1,1)    | Interfaz discreto-continuo, causal     | 4           |
| 2     | Cl(2,0)    | Rotor funcional, transformaciones 2D   | 4           |
| 3     | Cl(3,0)    | CORE activo: campo, motifs, AMR        | 8           |
| 4     | Cl(2,1)    | Shadow temporal, deriva                | 8           |
| 5     | Cl(4,1)    | Shadow comprimido (backends.py)        | 12-16       |
| 6     | Cl(5,1)    | Destino hadrones/color SU(3)           | 64          |

### Gamma_virt como lifting operator

La Matriz Gamma Virtual (gamma_virtual.py) actua como proyector entre niveles
de la serie CT. Cuando chi > chi*, levanta tensiones del slice Cl(3,0) al
shadow Cl(4,1). El lifting cycle Phi->T->Psi es el mecanismo formal.

### Modos rotacionales en Cl(3,0)

Los 3 bivectores e12, e13, e23 son los unicos modos rotacionales puros del
espacio 3D. El pseudoescalar e123=I actua como imaginario y es el conector
natural hacia slices superiores.

### REGLA CRITICA: c_eff != c fisico

- c_eff (ODS) = limite Nyquist de la malla, depende del hardware
- c fisico (~3e8 m/s) requiere Cl(1,3) (Minkowski) — fuera scope P03-P05
- Gemini confundio estos dos en sesion 2026-03-24 — NO reproducir en papers

### Estado actual

- Serie CT IMPLICITA en runtime — no existe SliceSpec/SliceState explicito
- structural_genome.py ya existe como frontend discreto experimental
- Cl(4,1)-compressed ya en backends.py como shadow target

### Que falta para P04

1. SliceSpec/SliceState con pesos por slice
2. Lifting operators explicitos Cl(n) -> Cl(n+1)
3. Tests coherencia cross-slice via Gamma_virt
4. Conexion formal c_eff vs c fisico (Cl(1,3))
